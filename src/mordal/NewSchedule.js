    ...
    const handleSubmit = async () => {
        ...
        for (const modifiedPlan of modifyPlans) {
            ...
            if (matchingRecommend && (matchingRecommend.startTime !== modifiedPlan.startTime || matchingRecommend.endTime !== modifiedPlan.endTime)) {
                ...
                if (original.DEADLINE < modified.DEADLINE) {deadline += 0.05};
                if (original.DISTANCE < modified.DISTANCE) {distance += 0.05};
                if (original.FOCUS_TIME < modified.FOCUS_TIME) {focusTime += 0.05};
                if (original.NOT_FOCUS_TIME < modified.NOT_FOCUS_TIME) {notFocusTime += 0.05};
                if (original.PRIORITY < modified.PRIORITY) {priority += 0.05};
        ...
        try {
            ...
            await axios.patch(
                `${requests.fetchMember}`, myRecommendCount + 1,
                {
                    headers: {
                        Authorization: `Bearer ${accessToken}`, // accessToken을 헤더에 추가
                    },
